...
if __name__ == "__main__":
    main()
'''    
マイクロマウスのシミュレーションプログラムをM1MacBookProさんで試していたのをUbuntuさんでも試してみた。
依存関係をpipで入れればほぼ動くんですが、しばらくすると「応答がない」との警告が出る。Macでは出ない現象。
pygameのWindowを表示しているループから抜けてplt.show()で開いたmatplotlibのWindowを出したあとpygameを更新できないせいだ、と思っているのだけれどUbuntuのGUIの仕組みがわかっていないので今の所対処できていない。
pygameのWindowを先に閉じるようにすると問題は起きない。グラフのWindowが開いたあとはグラフ閉じるまでプログラムでは何もできないので対処できない。グラフを開いてもプログラムが次に進むようなことができれば解決できそう。
'''